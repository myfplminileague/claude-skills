...
**Escalation lifts the model. It cannot lift the effort.** Effort lives in a static per-role agent definition, so there is no call-time dial to turn — an escalated `standards` runs `opus`/`medium`, not `opus`/`high`, and the report will say so. Read the tier table's effort column as *the role's setting*, not as a property of the tier that escalation carries with it: `prover` already sits at top tier on `medium` deliberately, so tier and effort were never a fixed pair. This is the intended behaviour, not a shortfall, and the reasoning is worth keeping because a reviewer once blocked on it: **effort tracks what the brief asks of a role, not which model is running it.** Escalating `standards` because no pack matched buys a stronger model for the same lookup work; it does not retroactively decide that role must now invent things. So `opus`/`medium` is the right answer there, and the report saying so is accurate rather than embarrassing. If you ever do want both dimensions raised, the only lever is a second definition — not worth building until a run demonstrably needed it.
...
**Silence fails safe.** A challenger that returns no verdict — idle, dead, or empty — leaves the finding **`block`, unchanged**. Retry once; if the second attempt is also silent, stop and mark the finding `unchallenged` in the report, **recording whether it timed out or genuinely returned nothing** — and give it more time than feels necessary before deciding, because a challenger reproducing a mutation is doing real work and takes longer than one that merely reads. A verdict that arrives after you stopped waiting still counts: reconcile it and correct the report. See *Known limits*, where declaring two live challengers dead produced a report wrong in two places. Never read an absent verdict as consent to demote. This is the opposite of the fan-out's rule and deliberately so: there, a missing report means work was not done, so the run is `INCOMPLETE`; here, a missing verdict would *delete* work already done. Deleting a finding requires an affirmative refutation, never the absence of one.
...
- **Agents go silent** — reviewers and challengers both sometimes finish without returning anything, not uniformly across models. Every stage therefore has an explicit rule for absence: `DID NOT REPORT` in the fan-out, `unchallenged` in the refute pass. Neither ever reads as approval. If a run reports neither, the orchestrator skipped a check.
- **But slow is not silent, and the orchestrator cannot tell them apart.** In the third run both challengers were declared silent and their findings shipped `unchallenged` — then both verdicts arrived, complete and well-reasoned, about ten minutes after the wait window closed. One `STANDS` with an independent reproduction, one `REFUTED` that overturned a block. The fail-safe rule worked exactly as designed and still produced a report that was wrong in two places, because *timed out* had been written down as *returned nothing*.

  Three consequences, and they are the practical ones:

  - **Say which it was.** `unchallenged (timed out after <n> min)` and `unchallenged (returned empty)` are different claims about the world. Only the second is evidence about the agent.
  - **A late verdict is not stale — reconcile it.** A refutation that lands after the report is written still refutes. Reopen the finding, demote it, and correct the record; do not discard a verdict for arriving inconveniently. A report is a claim about the code, not a snapshot of who had replied by then.
  - **Never put a timeout in the permanent record as a behavioural finding.** The first draft of this very bullet asserted that both challengers "returned no verdict", and that assertion was merged into this skill before the verdicts arrived — a false claim about the system, in the document people consult to decide how much to trust it. Wait longer than feels necessary before concluding an agent is dead.
